...
int vertex_pos(TChain *mychain, TString era){
    ...
    TCanvas *c1 = new TCanvas("c1", "c1", 800, 600); canvas_setup(c1); CMS_lumi(c1);
    h1->SetMinimum(3);
    h1->Draw("COLZ");
    ...
    c1->SetLogz();
    c1->Update(); c1->RedrawAxis(); c1->SaveAs(figpath1);
    delete h1; delete c1;
    ...
    TCanvas *c4 = new TCanvas("c4", "c4", 800, 600); canvas_setup(c4); CMS_lumi(c4);
    h4->SetMinimum(3);
    h4->Draw("COLZ");
    write_text(0.5, 0.85, datatype_text);
    c4->Update(); c4->RedrawAxis(); c4->SaveAs(figpath4);
    delete h4; delete c4; 
    ...
    TCanvas *c5 = new TCanvas("c5", "c5", 800, 600); canvas_setup(c5); CMS_lumi(c5);
    h5->SetMinimum(3);
    h5->Draw("COLZ");
    write_text(0.5, 0.85, datatype_text);
    c5->Update(); c5->RedrawAxis(); c5->SaveAs(figpath5);
    delete h5; delete c5; 
    ...
    TCanvas *c6 = new TCanvas("c6", "c6", 800, 600); canvas_setup(c6); CMS_lumi(c6);
    h6->SetMinimum(3);
    h6->Draw("COLZ");
    write_text(0.5, 0.85, datatype_text);
    c6->Update(); c6->RedrawAxis(); c6->SaveAs(figpath6);
    delete h6; delete c6; 
    ...
    TString figpath7 = fig_dir + "/bs_x.png";
    mychain->Project("h7", "bs_x*10");
    TCanvas *c7 = new TCanvas("c7", "c7", 800, 600); canvas_setup(c7); CMS_lumi(c7);
    h7->SetMaximum(1.3*h7->GetMaximum());
    h7->Draw();
    write_text(0.5, 0.85, datatype_text);
    c7->Update(); c7->RedrawAxis(); c7->SaveAs(figpath7);
    delete h7; delete c7;

    TH1F *h8 = new TH1F("h8", ";y [mm];# beamspots", 200, bsymin, bsymax);
    TString figpath8 = fig_dir + "/bs_y.png";
    mychain->Project("h8", "bs_y*10");
    TCanvas *c8 = new TCanvas("c8", "c8", 800, 600); canvas_setup(c8); CMS_lumi(c8);
    h8->SetMaximum(1.3*h8->GetMaximum());
    h8->Draw();
    write_text(0.5, 0.85, datatype_text);
    c8->Update(); c8->RedrawAxis(); c8->SaveAs(figpath8);
    delete h8; delete c8; 

    TH1F *h9 = new TH1F("h9", ";z [mm];# beamspots", 200, bszmin, bszmax);
    TString figpath9 = fig_dir + "/bs_z.png";
    mychain->Project("h9", "bs_z*10");
    TCanvas *c9 = new TCanvas("c9", "c9", 800, 600); canvas_setup(c9); CMS_lumi(c9);
    h9->SetMaximum(1.3*h9->GetMaximum());
    h9->Draw();
    write_text(0.5, 0.85, datatype_text);
    c9->Update(); c9->RedrawAxis(); c9->SaveAs(figpath9);
    delete h9; delete c9; 
}
...
int vertex_analysis() {
    ...
    for(int i=0; i<nera; i++){
        ...
        mychain->Add(Form("%svertex_analysis.root", filepath[i].c_str()));
        all_chain->Add(Form("%svertex_analysis.root", filepath[i].c_str()));
