...
template <typename F>
void process_tree_tracks_data(TChain *tree,
                              Long64_t nEntries,
                              Int_t idx,
...
Int_t ip_res_claude(TString period, Int_t idx)
{
    // 输出目录
    ...
    auto bind_track_branches = [&](TChain *tree)
    {
    ...
    // ---------- Count exclusive trigger pass events in data ----------
    ...
    for (int it = 0; it < N_TRIGS; ++it)
    {
        ps_weights_final[it] = (n_data_excl[it] > 0)
                                   ? L_eff[it] / static_cast<double>(n_data_excl[it])
                                   : 0.0;
    }

    // ---------- Histogram titles & paths ----------
    ...
    process_tree_tracks_data(datatree,
                             nData,
                             idx,
                             pv_trk_pt_edges,
    ...
    for (auto &mc_entry : tuplelist[period.Data()]["mc"])
    {
        ...
        TChain *runinfo = new TChain("residuals/RunInfo");
        ...
        bind_track_branches(mctree_entry);
        ...
        // --- Count exclusive trigger pass events in this MC entry ---
        ...
        // --- Per-entry PS weights with mask ---
        double ps_weights_entry[N_TRIGS];
        for (int it = 0; it < N_TRIGS; ++it)
        {
            ps_weights_entry[it] = (n_mc_excl[it] < 0.01 * static_cast<double>(total_nEventsProcessed))
                                       ? 0.0
                                       : ps_weights_final[it];
        }
        ...
        for (Long64_t ie = 0; ie < nMC_entry; ++ie)
        {
            ...
            // PS weight: highest-priority unmasked trigger
            Float_t w_ps = 0.0;
            for (int it = 0; it < N_TRIGS; ++it)
            {
                if (trig_pass_raw[it])
            ...
            Float_t w_event = w_pu * xsec_weight * w_ps;
            ...
            for (size_t it = 0; it < nTrk; ++it)
            {
                ...
                {
                    if (h_d0_mc[cid])
                        h_d0_mc[cid]->Fill(d0v, w_event);
                    if (h_dz_mc[cid])
                        h_dz_mc[cid]->Fill(dzv, w_event);
                };
