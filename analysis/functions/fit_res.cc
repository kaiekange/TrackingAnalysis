...
std::pair<float, float> fit_compare(TH1F * hist_data, TH1F * hist_mc, TString era, TString figpath_data, TString figpath_mc, float tolerance = 1e-4) {
    ...
    RooPlot *frame_data = pv_var.frame();

    hdatahist_data.plotOn(frame_data, RooFit::Name("data"), RooFit::MarkerColor(kBlack), RooFit::MarkerSize(1.1), RooFit::Binning(100), RooFit::DrawOption("ep"));
    ...
    RooPlot *frame_mc = pv_var.frame();

    hdatahist_mc.plotOn(frame_mc, RooFit::Name("mc"), RooFit::MarkerColor(kBlack), RooFit::MarkerSize(1.1), RooFit::Binning(100), RooFit::DrawOption("ep"));
